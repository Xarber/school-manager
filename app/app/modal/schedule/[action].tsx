...
async function updatePeriod({day, classData, period, setLoading, alert, mode}: updatePeriodProps) {
    setLoading(true);
    ...
    currentSchedule.hours = currentSchedule.hours.filter((h: any) => h.startTime !== period.startTime || h.endTime !== period.endTime);
    ...
    classData.save({
        ...
        schedule: [
            ...filteredSchedule,
            {
                ...
                ].filter(e => e !== null).sort((a: any, b: any) => {
        ...
        alert.show({title: i18n.t(`modal.schedule.${mode}.success.title`), message: i18n.t(`modal.schedule.${mode}.success.description`), actions: [
            {
                title: i18n.t(`modal.schedule.${mode}.success.confirm`),
                onPress: () => {
                    alert.hide();
                    router.back();
                }
        ...
        alert.show({title: i18n.t(`modal.schedule.${mode}.error.title`), message: err});
        setLoading(false);
    })
}
...
function EditSchedule() {
    ...
    const [h1, m1] = editStartTime ? editStartTime.split(":").map(Number) : [9, 0];
    const [h2, m2] = editEndTime ? editEndTime.split(":").map(Number) : [10, 0];
    ...
    const [periodSubjects, setPeriodSubjects] = useState<string[]>(
        mode === "edit" ? 
            classData.data.schedule.find((s: {day: number}) => s.day === day)?.hours.find((h: {startTime: string, endTime: string}) => h.startTime === editStartTime && h.endTime === editEndTime)?.subjects ?? []
        : []
    );
    ...
    const periodTimelineData = {
        ...periodData,
        items: (periodSubjects).map((e: string)=>{
    ...
    return (
        <>
            <Stack.Screen options={{headerTitle: i18n.t(`modal.schedule.${mode}.stack.title`)}} />
            {
                    ...
                    <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingBottom: bottomHeight}}>
                        <View style={[commonStyle.dashboardSection, modalStyle.container]}>
                            <View style={modalStyle.cardDetails}>
                                ...
                                <Timeline periods={[periodTimelineData]} />
                                <Text style={commonStyle.text}>{i18n.t(`modal.schedule.${mode}.createdon.text`, {createdOn: new Date().toDateString()})}</Text>
                                <Text style={commonStyle.text}>{i18n.t(`modal.schedule.${mode}.merge.text`, {className: classData.data.name})}</Text>
                            </View>
                            <View style={modalStyle.cardEdit}>
                                {/* <View style={{ display: "flex", flexDirection: "row", gap: 10}}> */}
                                    <View style={[modalStyle.cardEditField, {flex: 1}]}>
                                        <Text style={[commonStyle.headerText, modalStyle.cardEditFieldText]}>{i18n.t(`modal.schedule.${mode}.datetime.startTitle`)}</Text>
                                        {
                                            Platform.OS === "android" && (
                                                <View style={{display: "flex", flexDirection: "row", gap: 10}}>
                                    ...
                                    <View style={[modalStyle.cardEditField, {flex: 1}]}>
                                        <Text style={[commonStyle.headerText, modalStyle.cardEditFieldText]}>{i18n.t(`modal.schedule.${mode}.datetime.endTitle`)}</Text>
                                        {
                                            Platform.OS === "android" && (
                                                <View style={{display: "flex", flexDirection: "row", gap: 10}}>
                                ...
                                <View style={modalStyle.cardEditField}>
                                    <Text style={[commonStyle.headerText, modalStyle.cardEditFieldText]}>{i18n.t(`modal.schedule.${mode}.subjects.title`)}</Text>
                                    <View style={{ gap: 10 }}>
                                    ...
                                    <View style={modalStyle.cardEditField}>
                                        <Text style={modalStyle.cardEditFieldText}>{i18n.t(`modal.schedule.${mode}.subjects.addtitle`)}</Text>
                                                ...
                                                Platform.OS === "ios" && (
                                                    <>
                                                        <TouchableOpacity style={[modalStyle.cardEditFieldSelect, {flex: 1, width: "auto"}]} onPress={()=>setSubjectPickeriOSvisible(true)}>
                                                            <Text style={modalStyle.cardEditFieldSelectText}>{subjects.find((e: SubjectData)=>e._id === addingPeriod)?.name ?? i18n.t(`modal.schedule.${mode}.subjects.select`)}</Text>
